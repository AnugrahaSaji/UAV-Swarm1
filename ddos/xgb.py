...
runs the pre-trained XGBoost model for multi-class DDoS detection.

The model classifies 15 classes (14 attack types + BenignTraffic)
from the CIC-IoT-2023 dataset — same pipeline as LightGBM and RF.

Usage (requires root for packet capture):
    sudo python xgb.py
    sudo python xgb.py --iface eth0 --window 100 --threshold 0.5
"""
...
MODEL_PATH = os.path.join(_DIR, "models", "xgb_model.pkl")
...
def load_model():
    """Load XGBoost model, scaler, and label mapping from bundled pickle.
...
def main():
    ...
    print(f"[XGB] Loading model from {MODEL_PATH}")
    model, scaler, id_to_label = load_model()
    print(f"[XGB] Model loaded — {len(id_to_label)} classes, "
    ...
    print(f"[XGB] Sniffing on '{args.iface}'... "
