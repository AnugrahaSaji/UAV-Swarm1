#!/usr/bin/env python3
"""
Live RandomForest DDoS Detector — Tier 2 Confirmer
===================================================
Sniffs network traffic, extracts CIC-IoT-2023 flow features, and
runs the pre-trained RandomForest model for high-accuracy multi-class
DDoS detection.

Tier 2 in the 3-tier cascade — activated by the scheduler when
Tier 1 (LightGBM) raises a detection alert and confirmation is needed.
...
Usage (requires root for packet capture):
    sudo python rf.py
    sudo python rf.py --iface eth0 --window 100 --threshold 0.5
"""
...
MODEL_PATH = os.path.join(_DIR, "models", "rf_model.pkl")
...
def load_model():
    """Load RandomForest model, scaler, and label mapping from bundled pickle.
...
def main():
    parser = argparse.ArgumentParser(description="Live RandomForest DDoS detector (Tier 2)")
    ...
    try:
        ...
        print(f"\n[RF] Stopped after {window_num} windows.")
