...
def _validate_signing_identity(
    ...
    *,
    ...
    """Fail fast when the loaded signer does not match the configured public key."""

    if public_key is None:
        return
    if not hasattr(signer, "sign"):
        raise TypeError("GCS signing object must provide sign()")

    signature_type = signature_cls or _require_signature_class()
    probe = b"pq-drone-gcs:gcs-signing-identity-self-test:v1"

    try:
        signature = signer.sign(probe)
    except Exception as exc:
        raise RuntimeError(f"GCS signing identity self-test failed during sign(): {exc}") from exc

    verifier = None
    try:
        verifier = signature_type(sig_name)
        try:
            verified = verifier.verify(probe, signature, public_key)
        except Exception as exc:
            raise RuntimeError(f"GCS signing identity self-test failed during verify(): {exc}") from exc
        if not verified:
            raise RuntimeError(
                "Loaded GCS signing identity does not match the configured public key. "
                "This oqs build may not support reloading persistent secret keys safely; "
                "use --ephemeral for testing or regenerate/load the identity on a build "
                "with working secret-key import support."
            )
    finally:
        if verifier is not None and hasattr(verifier, "free"):
            try:
                verifier.free()
            except Exception:
                pass
...
def _apply_control_overrides(cfg: dict, args) -> dict:
    """Return a cfg copy with --enable-tcp-control / --coordinator-role applied."""
    enable_tcp = getattr(args, "enable_tcp_control", False)
    coord_role = getattr(args, "coordinator_role", None)
    if not enable_tcp and not coord_role:
        return cfg  # no overrides → reuse original dict
    cfg = dict(cfg)
    if enable_tcp:
        cfg["ENABLE_TCP_CONTROL"] = True
    if coord_role:
        cfg["CONTROL_COORDINATOR_ROLE"] = coord_role
    return cfg
...
def _resolve_suite(args, role_label: str) -> dict:
    ...
    if not suite_arg:
        print(
            f"Error: {role_label} requires --suite (example: {DEFAULT_SUITE_ID})"
        )
        sys.exit(1)
    try:
...
def _apply_aead_override(args, suite: dict) -> dict:
    """Return a suite-like dict with an explicit runtime AEAD override applied."""
    requested = getattr(args, "aead", None)
    if not requested:
        return dict(suite)

    try:
        aead_token = normalize_aead_token_for_level(
            str(requested),
            str(suite.get("nist_level", "")),
        )
    ...
    resolved = dict(suite)
    resolved["aead_token"] = aead_token
    resolved["data_aead_id"] = f"dap-{aead_token}"
    resolved["data_aead"] = aead_token
    resolved["aead"] = aead_token
    setattr(args, "aead", aead_token)
    return resolved


def _preflight_drone_psk(cfg: dict) -> None:
    """Fail closed when DRONE_PSK is missing or malformed."""

    psk_hex = os.getenv("DRONE_PSK", cfg.get("DRONE_PSK", ""))
    if not isinstance(psk_hex, str) or not psk_hex.strip():
        raise RuntimeError("DRONE_PSK must be provided and decode to 32 bytes")
    try:
        psk_bytes = bytes.fromhex(psk_hex)
    except ValueError as exc:
        raise RuntimeError(f"Invalid DRONE_PSK hex: {exc}") from exc
    if len(psk_bytes) != 32:
        raise RuntimeError("DRONE_PSK must decode to 32 bytes")


def _is_tailscale_host(host: object) -> bool:
    """Return True for canonical Tailscale CGNAT addresses (100.x.x.x)."""
    ...
    if not value:
        return False
    return value.startswith("100.")


def _preflight_runtime_hosts(cfg: dict) -> None:
    """Reject maintenance-plane host addresses for runtime traffic by default."""

    allow = os.getenv("ALLOW_TAILSCALE_RUNTIME", "").strip().lower() in {"1", "true", "yes", "on"}
    if allow:
        return

    drone_host = cfg.get("DRONE_HOST")
    gcs_host = cfg.get("GCS_HOST")
    offenders = []
    if _is_tailscale_host(drone_host):
        offenders.append(f"DRONE_HOST={drone_host}")
    if _is_tailscale_host(gcs_host):
        offenders.append(f"GCS_HOST={gcs_host}")
    if offenders:
        details = ", ".join(offenders)
        raise RuntimeError(
            "Runtime host(s) resolved to Tailscale maintenance plane: "
            f"{details}. Use LAN hosts for data/control/telemetry runtime, "
            "or set ALLOW_TAILSCALE_RUNTIME=1 for intentional override."
        )


def _run_startup_preflight(role: str, cfg: dict, suite: dict) -> None:
    """Validate startup invariants before entering proxy runtime."""
    ...
    required_suite_keys = ("suite_id", "kem_name", "sig_name", "aead")
    missing_suite_keys = [k for k in required_suite_keys if k not in suite]
    if missing_suite_keys:
        missing = ", ".join(missing_suite_keys)
        raise RuntimeError(f"Resolved suite is missing required fields: {missing}")

    if role not in {"gcs", "drone"}:
        raise RuntimeError(f"Unsupported role for preflight: {role}")

    _preflight_runtime_hosts(cfg)
    _preflight_drone_psk(cfg)


def _validate_preflight_role_inputs(role: str, args) -> None:
    """Validate CLI-provided key material inputs without starting proxy runtime."""

    if role == "gcs":
        if getattr(args, "ephemeral", False):
            return
        secret_path = Path(getattr(args, "gcs_secret_file", "") or "secrets/gcs_signing.key")
        if not secret_path.exists():
            raise RuntimeError(
                f"GCS preflight failed: secret key file not found: {secret_path}"
            )
        try:
            _ = secret_path.read_bytes()
        except Exception as exc:
            raise RuntimeError(
                f"GCS preflight failed: cannot read secret key file {secret_path}: {exc}"
            ) from exc
        return

    if role == "drone":
        pub_hex = getattr(args, "gcs_pub_hex", None)
        if pub_hex:
            try:
                _ = bytes.fromhex(pub_hex)
            except ValueError as exc:
                raise RuntimeError(f"Drone preflight failed: invalid --gcs-pub-hex: {exc}") from exc
            return

        pub_path = Path(getattr(args, "peer_pubkey_file", "") or "secrets/gcs_signing.pub")
        if not pub_path.exists():
            raise RuntimeError(
                "Drone preflight failed: public key file not found: "
                f"{pub_path}. Use --peer-pubkey-file or --gcs-pub-hex."
            )
        try:
            _ = pub_path.read_bytes()
        except Exception as exc:
            raise RuntimeError(
                f"Drone preflight failed: cannot read public key file {pub_path}: {exc}"
            ) from exc
        return

    raise RuntimeError(f"Unsupported preflight role: {role}")


def preflight_command(args) -> None:
    """Run startup preflight checks without launching proxy runtime."""

    role = getattr(args, "role", None)
    if role not in {"gcs", "drone"}:
        print("Error: --role must be either 'gcs' or 'drone'")
        sys.exit(1)

    suite = _apply_aead_override(args, _resolve_suite(args, f"{role.upper()} preflight"))
    effective_cfg = _apply_control_overrides(CONFIG, args)

    try:
        _run_startup_preflight(role, effective_cfg, suite)
        _validate_preflight_role_inputs(role, args)
    except Exception as exc:
        print(f"Error: {exc}")
        sys.exit(1)

    data_aead_id = str(suite.get("data_aead_id", f"dap-{suite.get('aead_token', 'aesgcm')}"))

    print(
        "Preflight OK:",
        f"role={role}",
        f"suite={suite['suite_id']}",
        f"key_handshake={suite.get('key_handshake_id', '')}",
        f"data_aead={data_aead_id}",
        f"gcs_host={effective_cfg['GCS_HOST']}",
        f"drone_host={effective_cfg['DRONE_HOST']}",
    )
...
def gcs_command(args):
    """Start GCS proxy."""
    suite = _apply_aead_override(args, _resolve_suite(args, "GCS proxy"))
    ...
    if args.ephemeral:
        info("WARNING: Using EPHEMERAL keys - not suitable for production!")
        info("WARNING: Key will be lost when process exits.")
        if not quiet:
            print()
    ...
    effective_cfg = _apply_control_overrides(CONFIG, args)
    _run_startup_preflight("gcs", effective_cfg, suite)
...
def drone_command(args):
    """Start drone proxy."""
    suite = _apply_aead_override(args, _resolve_suite(args, "Drone proxy"))
    suite_id = suite["suite_id"]
    ...
    try:
        ...
        effective_cfg = _apply_control_overrides(CONFIG, args)
        _run_startup_preflight("drone", effective_cfg, suite)
...
def main():
    ...
    gcs_parser.add_argument("--aead", help="Runtime AEAD token override for the selected suite")
    ...
    gcs_parser.add_argument("--enable-tcp-control", action="store_true",
                           help="Enable TCP control server for external rekey commands")
    gcs_parser.add_argument("--coordinator-role", choices=["drone", "gcs"],
                           help="Override CONTROL_COORDINATOR_ROLE (default: from config)")
    ...
    drone_parser.add_argument("--aead", help="Runtime AEAD token override for the selected suite")
    ...
    drone_parser.add_argument("--control-manual", action="store_true",
                              help="Enable interactive manual in-band rekey control thread")
    drone_parser.add_argument("--enable-tcp-control", action="store_true",
                              help="Enable TCP control server for external rekey commands")
    drone_parser.add_argument("--coordinator-role", choices=["drone", "gcs"],
                              help="Override CONTROL_COORDINATOR_ROLE (default: from config)")

    # preflight subcommand
    preflight_parser = subparsers.add_parser('preflight', help='Validate startup configuration and inputs without starting proxy')
    preflight_parser.add_argument("--role", choices=["gcs", "drone"], required=True,
                                  help="Target role to validate")
    preflight_parser.add_argument("--suite", help="Cryptographic suite ID")
    preflight_parser.add_argument("--aead", help="Runtime AEAD token override for the selected suite")
    preflight_parser.add_argument("--gcs-secret-file",
                                  help="Path to GCS secret key file (gcs role, default: secrets/gcs_signing.key)")
    preflight_parser.add_argument("--ephemeral", action="store_true",
                                  help="Skip persistent secret file checks for gcs role")
    preflight_parser.add_argument("--peer-pubkey-file",
                                  help="Path to GCS public key file (drone role, default: secrets/gcs_signing.pub)")
    preflight_parser.add_argument("--gcs-pub-hex",
                                  help="GCS public key as hex string (drone role)")
    preflight_parser.add_argument("--enable-tcp-control", action="store_true",
                                  help="Apply ENABLE_TCP_CONTROL override during validation")
    preflight_parser.add_argument("--coordinator-role", choices=["drone", "gcs"],
                                  help="Override CONTROL_COORDINATOR_ROLE during validation")
