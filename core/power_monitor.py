...
# INA219 bus-voltage gain correction.  The breakout at 0x40 on our Pi 4
# has a counterfeit/clone INA219 whose bus-voltage ADC systematically reads
# ~16% low (4.22 V instead of the true ~5.0 V confirmed by the Pi PMIC
# undervoltage alarm = 0 and throttled = 0x0).  Register writes are
# silently ignored so hardware calibration is impossible.  This software
# gain factor corrects the raw ADC reading.  Shunt voltage (and therefore
# current) is accurate and needs no correction.
# Set INA219_VBUS_GAIN=1.0 in the environment to disable correction.
# Calibrated 2025-01 against AVHzYCT3 factory-grade meter (short wires).
_VBUS_GAIN = float(os.getenv("INA219_VBUS_GAIN", "1.035"))
...
class Ina219PowerMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        # BCM2835 I2C first-transaction workaround: the first I2C_SMBUS ioctl
        # after opening /dev/i2c-N always returns EIO on RPi4.  A single
        # throwaway read_byte_data (which includes a register-select write
        # phase) clears the condition.  Using read_byte (no write phase)
        # would corrupt the bus state â€” only read_byte_data works.
        try:
            self._bus.read_byte_data(address, 0x00)
        except OSError:
    ...
    def _configure(self, sample_hz: float) -> None:
        """Attempt to write INA219 config register.

        NOTE: On our counterfeit/clone INA219 at 0x40, ALL register writes
        are silently ignored â€” the chip ACKs the I2C transaction but the
        register value stays at the power-on default 0x399F.  This was
        exhaustively verified with write_word_data, i2c_rdwr, raw os.write,
        kernel i2cset, and i2ctransfer.  The default 0x399F means:
          - 32 V bus range, PGA /8 (Â±320 mV â†’ Â±3.2 A with 0.1 Î© shunt)
          - 12-bit ADC, continuous shunt + bus mode
        which is acceptable for our measurement requirements.  The write is
        kept for correctness on genuine INA219 chips.
        """
        ...
        # Use write_word_data instead of write_i2c_block_data: the latter
        # uses I2C_SMBUS_I2C_BLOCK_BROKEN which permanently corrupts the
        # BCM2835 I2C bus state on RPi4.  write_word_data uses the safe
        # I2C_SMBUS_WORD_DATA command.  SMBus word order is little-endian
        # so we swap bytes (INA219 registers are big-endian).
        word_le = ((cfg & 0xFF) << 8) | ((cfg >> 8) & 0xFF)
        with self._bus_lock:
            self._bus.write_word_data(self.address, 0x00, word_le)  # type: ignore[union-attr]
        time.sleep(profile["settle"])
        # BCM2835 post-write warm-up: the first read after any write fails
        # with EIO.  A throwaway read_byte_data clears the condition.
        try:
            self._bus.read_byte_data(self.address, 0x00)  # type: ignore[union-attr]
        except OSError:
            pass
    ...
    def _read_bus_voltage(self) -> float:
        """Read INA219 bus voltage register (0x02) with gain correction.

        Raw formula: bits[15:3] Ã— 4 mV (INA219 datasheet Â§8.6.3).
        The counterfeit chip's ADC reads ~3.5 %% low on the bus voltage
        channel.  ``_VBUS_GAIN`` (default 1.035, calibrated against
        AVHzYCT3) corrects this.  Shunt
        voltage (register 0x01) is accurate and not gain-corrected.
        """
        raw = self._read_u16(0x02)
        return ((raw >> 3) & 0x1FFF) * 0.004 * _VBUS_GAIN

    def _read_u16(self, register: int) -> int:
        # Use read_word_data instead of read_i2c_block_data: the latter
        # uses I2C_SMBUS_I2C_BLOCK_DATA which is unreliable after writes
        # on BCM2835.  read_word_data uses I2C_SMBUS_WORD_DATA (safe).
        # SMBus returns little-endian; INA219 is big-endian -> swap bytes.
        # Retry up to 3 times: handles BCM2835 first-read-after-idle (EIO)
        # and the zero-return-after-idle bug (returns 0x0000 without error).
        with self._bus_lock:
            last_err: OSError | None = None
            for _attempt in range(3):
                try:
                    word_le = self._bus.read_word_data(self.address, register)  # type: ignore[union-attr]
                    val = ((word_le & 0xFF) << 8) | ((word_le >> 8) & 0xFF)
                    # BCM2835 idle bug: first read after >~50ms idle returns 0.
                    # Retry for bus voltage (0x02) and config (0x00) registers
                    # which should never be 0 during normal operation.
                    if val == 0 and register in (0x00, 0x02) and _attempt < 2:
                        continue
                    return val
...
class Rpi5PmicPowerMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        # Probe pmic_read_adc to fail fast on Pi 4 (BCM2711) where vcgencmd
        # exists but pmic_read_adc returns "Command not registered".  Without
        # this probe, create_power_monitor(backend="auto") would return a
        # broken Rpi5PmicPowerMonitor object that only fails at first capture().
        try:
            self._read_once()
        except PowerMonitorUnavailable as exc:
            raise PowerMonitorUnavailable(
                f"pmic_read_adc probe failed (Pi 4 / BCM2711?): {exc}"
            ) from exc
